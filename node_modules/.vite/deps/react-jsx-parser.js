...
//#region node_modules/react-jsx-parser/dist/react-jsx-parser.min.js
...
var require_acorn = __commonJS((exports, module) => {
	(function(global, factory) {
		...
		function isIdentifierStart2(code, astral) {
			...
			if (code <= 65535) return code >= 170 && nonASCIIidentifierStart2.test(String.fromCharCode(code));
		...
		function isIdentifierChar2(code, astral) {
			...
			if (code < 97) return code === 95;
			if (code < 123) return true;
			if (code <= 65535) return code >= 170 && nonASCIIidentifier2.test(String.fromCharCode(code));
			if (astral === false) return false;
		...
		function nextLineBreak2(code, from, end) {
			...
			for (var i3 = from; i3 < end; i3++) {
				...
				if (isNewLine2(next)) return i3 < end - 1 && next === 13 && code.charCodeAt(i3 + 1) === 10 ? i3 + 2 : i3 + 1;
			}
			return -1;
		}
		...
		function getLineInfo2(input, offset2) {
			for (var line = 1, cur = 0;;) {
				...
				if (nextBreak < 0) return new Position3(line, offset2 - cur);
				++line;
				cur = nextBreak;
			}
		}
		...
		function getOptions2(opts) {
			var options = {};
			for (var opt in defaultOptions2) options[opt] = opts && hasOwn2(opts, opt) ? opts[opt] : defaultOptions2[opt];
...
Defaulting to 2020, but this will stop working in the future.`);
			...
			if (isArray2(options.onComment)) options.onComment = pushComment2(options, options.onComment);
			return options;
		}
		function pushComment2(options, array) {
			return function(block, text, start, end, startLoc, endLoc) {
				...
				if (options.locations) comment.loc = new SourceLocation3(this, startLoc, endLoc);
...
`, startPos - 1) + 1;
		...
		prototypeAccessors2.canAwait.get = function() {
			for (var i3 = this.scopeStack.length - 1; i3 >= 0; i3--) {
				...
				if (scope.inClassFieldInit || scope.flags & SCOPE_CLASS_STATIC_BLOCK2) return false;
				if (scope.flags & SCOPE_FUNCTION2) return (scope.flags & SCOPE_ASYNC2) > 0;
			}
		...
		Parser3.extend = function extend() {
			...
			var cls = this;
			for (var i3 = 0; i3 < plugins.length; i3++) cls = plugins[i3](cls);
			return cls;
		};
		...
		pp$92.semicolon = function() {
			if (!this.eat(types$12.semi) && !this.insertSemicolon()) this.unexpected();
		};
		...
		pp$82.parseTopLevel = function(node) {
			...
			if (this.inModule) for (var i3 = 0, list3 = Object.keys(this.undefinedExports); i3 < list3.length; i3 += 1) {
				var name = list3[i3];
		...
		pp$82.isLet = function(context) {
			...
			if (isIdentifierStart2(nextCh, true)) {
				var pos = next + 1;
				while (isIdentifierChar2(nextCh = this.input.charCodeAt(pos), true)) ++pos;
				if (nextCh === 92 || nextCh > 55295 && nextCh < 56320) return true;
				var ident = this.input.slice(next, pos);
				if (!keywordRelationalOperator2.test(ident)) return true;
			}
			return false;
		};
		...
		pp$82.parseStatement = function(context, topLevel, exports3) {
			...
			switch (starttype) {
				...
				case types$12._continue: return this.parseBreakContinueStatement(node, starttype.keyword);
				case types$12._debugger: return this.parseDebuggerStatement(node);
				case types$12._do: return this.parseDoStatement(node);
				case types$12._for: return this.parseForStatement(node);
				...
				case types$12._if: return this.parseIfStatement(node);
				case types$12._return: return this.parseReturnStatement(node);
				case types$12._switch: return this.parseSwitchStatement(node);
				case types$12._throw: return this.parseThrowStatement(node);
				case types$12._try: return this.parseTryStatement(node);
				case types$12._const:
				case types$12._var:
					kind = kind || this.value;
					if (context && kind !== "var") this.unexpected();
					return this.parseVarStatement(node, kind);
				case types$12._while: return this.parseWhileStatement(node);
				case types$12._with: return this.parseWithStatement(node);
				case types$12.braceL: return this.parseBlock(true, node);
				case types$12.semi: return this.parseEmptyStatement(node);
				...
				default:
					...
					if (starttype === types$12.name && expr.type === "Identifier" && this.eat(types$12.colon)) return this.parseLabeledStatement(node, maybeName, expr, context);
		...
		pp$82.parseBreakContinueStatement = function(node, keyword) {
			...
			this.next();
			if (this.eat(types$12.semi) || this.insertSemicolon()) node.label = null;
			else if (this.type !== types$12.name) this.unexpected();
			else {
			...
			if (i3 === this.labels.length) this.raise(node.start, "Unsyntactic " + keyword);
		...
		pp$82.parseDoStatement = function(node) {
			...
			if (this.options.ecmaVersion >= 6) this.eat(types$12.semi);
			else this.semicolon();
		...
		pp$82.parseForStatement = function(node) {
			...
			if (this.type === types$12._in || (isForOf = this.options.ecmaVersion >= 6 && this.isContextual("of"))) {
				if (awaitAt > -1) {
					if (this.type === types$12._in) this.unexpected(awaitAt);
					node.await = true;
		...
		pp$82.parseReturnStatement = function(node) {
			...
			this.next();
			if (this.eat(types$12.semi) || this.insertSemicolon()) node.argument = null;
			else {
		...
		pp$82.parseSwitchStatement = function(node) {
			...
			this.enterScope(0);
			var cur;
			for (var sawDefault = false; this.type !== types$12.braceR;) if (this.type === types$12._case || this.type === types$12._default) {
		...
		pp$82.parseThrowStatement = function(node) {
			this.next();
			if (lineBreak2.test(this.input.slice(this.lastTokEnd, this.start))) this.raise(this.lastTokEnd, "Illegal newline after throw");
			node.argument = this.parseExpression();
			this.semicolon();
		...
		pp$82.parseTryStatement = function(node) {
			...
			if (this.type === types$12._catch) {
				...
				this.next();
				if (this.eat(types$12.parenL)) clause.param = this.parseCatchClauseParam();
				else {
		...
		pp$82.parseLabeledStatement = function(node, maybeName, expr, context) {
			for (var i$1 = 0, list3 = this.labels; i$1 < list3.length; i$1 += 1) if (list3[i$1].name === maybeName) this.raise(expr.start, "Label '" + maybeName + "' is already declared");
		...
		pp$82.parseVar = function(node, isFor, kind, allowMissingInitializer) {
			...
			for (;;) {
				...
				if (this.eat(types$12.eq)) decl.init = this.parseMaybeAssign(isFor);
				else if (!allowMissingInitializer && kind === "const" && !(this.type === types$12._in || this.options.ecmaVersion >= 6 && this.isContextual("of"))) this.unexpected();
				else if (!allowMissingInitializer && decl.id.type !== "Identifier" && !(isFor && (this.type === types$12._in || this.isContextual("of")))) this.raise(this.lastTokEnd, "Complex binding patterns require an initialization value");
				else decl.init = null;
				node.declarations.push(this.finishNode(decl, "VariableDeclarator"));
				if (!this.eat(types$12.comma)) break;
			}
			return node;
		};
		...
		pp$82.parseFunction = function(node, statement, allowExpressionBody, isAsync, forInit) {
			...
			if (this.options.ecmaVersion >= 9 || this.options.ecmaVersion >= 6 && !isAsync) {
				if (this.type === types$12.star && statement & FUNC_HANGING_STATEMENT2) this.unexpected();
			...
			if (statement & FUNC_STATEMENT2) {
				...
				if (node.id && !(statement & FUNC_HANGING_STATEMENT2)) this.checkLValSimple(node.id, this.strict || node.generator || node.async ? this.treatFunctionsAsVar ? BIND_VAR2 : BIND_LEXICAL2 : BIND_FUNCTION2);
			}
			...
			if (!(statement & FUNC_STATEMENT2)) node.id = this.type === types$12.name ? this.parseIdent() : null;
			...
			while (this.type !== types$12.braceR) {
				...
				if (element) {
					...
					if (element.type === "MethodDefinition" && element.kind === "constructor") {
						...
						hadConstructor = true;
					} else if (element.key && element.key.type === "PrivateIdentifier" && isPrivateNameConflicted2(privateNameMap, element)) this.raiseRecoverable(element.key.start, "Identifier '#" + element.key.name + "' has already been declared");
				}
			}
		...
		pp$82.parseClassElement = function(constructorAllowsSuper) {
			if (this.eat(types$12.semi)) return null;
			...
			if (this.eatContextual("static")) {
				...
				if (this.isClassElementNameStart() || this.type === types$12.star) isStatic = true;
				else keyName = "static";
			}
			node.static = isStatic;
			if (!keyName && ecmaVersion3 >= 8 && this.eatContextual("async")) {
				if ((this.isClassElementNameStart() || this.type === types$12.star) && !this.canInsertSemicolon()) isAsync = true;
				else keyName = "async";
			}
			if (!keyName && (ecmaVersion3 >= 9 || !isAsync) && this.eat(types$12.star)) isGenerator = true;
		...
		pp$82.parseClassMethod = function(method, isGenerator, isAsync, allowsDirectSuper) {
			...
			} else if (method.static && checkKeyName2(method, "prototype")) this.raise(key.start, "Classes may not have a static property named prototype");
		...
		pp$82.parseClassField = function(field) {
			if (checkKeyName2(field, "constructor")) this.raise(field.key.start, "Classes can't have a field named 'constructor'");
			else if (field.static && checkKeyName2(field, "prototype")) this.raise(field.key.start, "Classes can't have a static field named 'prototype'");
		...
		pp$82.parseClassId = function(node, isStatement) {
			if (this.type === types$12.name) {
				...
				if (isStatement) this.checkLValSimple(node.id, BIND_LEXICAL2, false);
			} else {
		...
		pp$82.parseExportAllDeclaration = function(node, exports3) {
			...
			if (this.type !== types$12.string) this.unexpected();
		...
		pp$82.parseExport = function(node, exports3) {
			this.next();
			if (this.eat(types$12.star)) return this.parseExportAllDeclaration(node, exports3);
			...
			if (this.shouldParseExportStatement()) {
				...
				else this.checkExport(exports3, node.declaration.id, node.declaration.id.start);
				node.specifiers = [];
				node.source = null;
			} else {
		...
		pp$82.checkExport = function(exports3, name, pos) {
			...
			if (hasOwn2(exports3, name)) this.raiseRecoverable(pos, "Duplicate export '" + name + "'");
			exports3[name] = true;
		};
		pp$82.checkPatternExport = function(exports3, pat) {
			var type = pat.type;
			if (type === "Identifier") this.checkExport(exports3, pat, pat.start);
			else if (type === "ObjectPattern") for (var i3 = 0, list3 = pat.properties; i3 < list3.length; i3 += 1) {
				var prop = list3[i3];
		...
		pp$82.parseExportSpecifiers = function(exports3) {
			...
			while (!this.eat(types$12.braceR)) {
				if (!first) {
					...
					if (this.afterTrailingComma(types$12.braceR)) break;
				} else first = false;
		...
		pp$82.parseImportSpecifiers = function() {
			...
			if (this.type === types$12.name) {
				...
				if (!this.eat(types$12.comma)) return nodes;
			}
		...
		pp$82.parseModuleExportName = function() {
			if (this.options.ecmaVersion >= 13 && this.type === types$12.string) {
				...
				if (loneSurrogate2.test(stringLiteral.value)) this.raise(stringLiteral.start, "An export name cannot include a lone surrogate.");
		...
		pp$72.parseRestBinding = function() {
			var node = this.startNode();
			this.next();
			if (this.options.ecmaVersion === 6 && this.type !== types$12.name) this.unexpected();
		...
		pp$72.parseBindingAtom = function() {
			if (this.options.ecmaVersion >= 6) switch (this.type) {
				...
				case types$12.braceL: return this.parseObj(true);
			}
		...
		pp$72.parseBindingList = function(close, allowEmpty, allowTrailingComma, allowModifiers) {
			...
			while (!this.eat(close)) {
				...
				else this.expect(types$12.comma);
				if (allowEmpty && this.type === types$12.comma) elts.push(null);
				else if (allowTrailingComma && this.afterTrailingComma(close)) break;
				else if (this.type === types$12.ellipsis) {
					var rest = this.parseRestBinding();
					this.parseBindingListItem(rest);
					elts.push(rest);
					if (this.type === types$12.comma) this.raiseRecoverable(this.start, "Comma is not permitted after the rest element");
					this.expect(close);
					break;
		...
		pp$72.parseMaybeDefault = function(startPos, startLoc, left) {
			...
			if (this.options.ecmaVersion < 6 || !this.eat(types$12.eq)) return left;
		...
		pp$72.checkLValSimple = function(expr, bindingType, checkClashes) {
			...
			switch (expr.type) {
				case "Identifier":
					...
					if (isBind) {
						if (bindingType === BIND_LEXICAL2 && expr.name === "let") this.raiseRecoverable(expr.start, "let is disallowed as a lexically bound name");
						if (checkClashes) {
							if (hasOwn2(checkClashes, expr.name)) this.raiseRecoverable(expr.start, "Argument name clash");
							checkClashes[expr.name] = true;
						}
						if (bindingType !== BIND_OUTSIDE2) this.declareName(expr.name, bindingType, expr.start);
					}
					break;
				case "ChainExpression":
		...
		pp$62.braceIsBlock = function(prevType) {
			...
			if (parent === types2.f_expr || parent === types2.f_stat) return true;
			if (prevType === types$12.colon && (parent === types2.b_stat || parent === types2.b_expr)) return !parent.isExpr;
			if (prevType === types$12._return || prevType === types$12.name && this.exprAllowed) return lineBreak2.test(this.input.slice(this.lastTokEnd, this.start));
			if (prevType === types$12._else || prevType === types$12.semi || prevType === types$12.eof || prevType === types$12.parenR || prevType === types$12.arrow) return true;
			if (prevType === types$12.braceL) return parent === types2.b_stat;
			if (prevType === types$12._var || prevType === types$12._const || prevType === types$12.name) return false;
		...
		pp$62.updateContext = function(prevType) {
			...
			if (type.keyword && prevType === types$12.dot) this.exprAllowed = false;
		...
		types$12.parenR.updateContext = types$12.braceR.updateContext = function() {
			...
			if (out === types2.b_stat && this.curContext().token === "function") out = this.context.pop();
		...
		types$12._function.updateContext = types$12._class.updateContext = function(prevType) {
			if (prevType.beforeExpr && prevType !== types$12._else && !(prevType === types$12.semi && this.curContext() !== types2.p_stat) && !(prevType === types$12._return && lineBreak2.test(this.input.slice(this.lastTokEnd, this.start))) && !((prevType === types$12.colon || prevType === types$12.braceL) && this.curContext() === types2.b_stat)) this.context.push(types2.f_expr);
			else this.context.push(types2.f_stat);
		...
		types$12.backQuote.updateContext = function() {
			if (this.curContext() === types2.q_tmpl) this.context.pop();
			else this.context.push(types2.q_tmpl);
			this.exprAllowed = false;
		};
		types$12.star.updateContext = function(prevType) {
			if (prevType === types$12._function) {
				var index = this.context.length - 1;
				if (this.context[index] === types2.f_expr) this.context[index] = types2.f_expr_gen;
				else this.context[index] = types2.f_gen;
			}
			this.exprAllowed = true;
		};
		...
		pp$52.parseExpression = function(forInit, refDestructuringErrors) {
			...
			if (this.type === types$12.comma) {
				...
				while (this.eat(types$12.comma)) node.expressions.push(this.parseMaybeAssign(forInit, refDestructuringErrors));
		...
		pp$52.parseMaybeAssign = function(forInit, refDestructuringErrors, afterLeftParse) {
			...
			if (this.type.isAssign) {
				...
				if (this.type === types$12.eq) left = this.toAssignable(left, false, refDestructuringErrors);
				...
				if (this.type === types$12.eq) this.checkLValPattern(left);
		...
		pp$52.parseExprOp = function(left, leftStartPos, leftStartLoc, minPrec, forInit) {
			...
			if (prec != null && (!forInit || this.type !== types$12._in)) {
				if (prec > minPrec) {
					...
					if (coalesce) prec = types$12.logicalAND.binop;
					var op = this.value;
					this.next();
					var startPos = this.start, startLoc = this.startLoc;
					var right = this.parseExprOp(this.parseMaybeUnary(null, false, false, forInit), startPos, startLoc, prec, forInit);
					var node = this.buildBinary(leftStartPos, leftStartLoc, left, right, op, logical || coalesce);
					if (logical && this.type === types$12.coalesce || coalesce && (this.type === types$12.logicalOR || this.type === types$12.logicalAND)) this.raiseRecoverable(this.start, "Logical expressions and coalesce expressions cannot be mixed. Wrap either by parentheses");
		...
		pp$52.parseMaybeUnary = function(refDestructuringErrors, sawUnary, incDec, forInit) {
			...
			if (this.isContextual("await") && this.canAwait) {
				...
				else if (this.strict && node.operator === "delete" && isLocalVariableAccess2(node.argument)) this.raiseRecoverable(node.start, "Deleting local variable in strict mode");
				else if (node.operator === "delete" && isPrivateFieldAccess2(node.argument)) this.raiseRecoverable(node.start, "Private fields can not be deleted");
				else sawUnary = true;
				expr = this.finishNode(node, update ? "UpdateExpression" : "UnaryExpression");
			} else if (!sawUnary && this.type === types$12.privateId) {
				if ((forInit || this.privateNameStack.length === 0) && this.options.checkPrivateFields) this.unexpected();
				expr = this.parsePrivateIdent();
				if (this.type !== types$12._in) this.unexpected();
			} else {
		...
		pp$52.parseSubscript = function(base, startPos, startLoc, noCalls, maybeAsyncArrow, optionalChained, forInit) {
			...
			if (computed || optional && this.type !== types$12.parenL && this.type !== types$12.backQuote || this.eat(types$12.dot)) {
				...
				} else if (this.type === types$12.privateId && base.type !== "Super") node.property = this.parsePrivateIdent();
		...
		pp$52.parseExprAtom = function(refDestructuringErrors, forInit, forNew) {
			if (this.type === types$12.slash) this.readRegexp();
			...
			switch (this.type) {
				case types$12._super:
					...
					node = this.startNode();
					this.next();
					if (this.type === types$12.parenL && !this.allowDirectSuper) this.raise(node.start, "super() call outside constructor of a subclass");
					if (this.type !== types$12.dot && this.type !== types$12.bracketL && this.type !== types$12.parenL) this.unexpected();
				...
				case types$12.name:
					...
					if (canBeArrow && !this.canInsertSemicolon()) {
						if (this.eat(types$12.arrow)) return this.parseArrowExpression(this.startNodeAt(startPos, startLoc), [id], false, forInit);
						if (this.options.ecmaVersion >= 8 && id.name === "async" && this.type === types$12.name && !containsEsc && (!this.potentialArrowInForAwait || this.value !== "of" || this.containsEsc)) {
							...
							if (this.canInsertSemicolon() || !this.eat(types$12.arrow)) this.unexpected();
				...
				case types$12.regexp:
					...
					return node;
				case types$12.num:
				case types$12.string: return this.parseLiteral(this.value);
				case types$12._null:
				case types$12._true:
				...
				case types$12._class: return this.parseClass(this.startNode(), false);
				case types$12._new: return this.parseNew();
				case types$12.backQuote: return this.parseTemplate();
				case types$12._import: if (this.options.ecmaVersion >= 11) return this.parseExprImport(forNew);
		...
		pp$52.parseExprImport = function(forNew) {
			...
			this.next();
			if (this.type === types$12.parenL && !forNew) return this.parseDynamicImport(node);
		...
		pp$52.parseDynamicImport = function(node) {
			...
			if (!this.eat(types$12.parenR)) {
				...
				if (this.eat(types$12.comma) && this.eat(types$12.parenR)) this.raiseRecoverable(errorPos, "Trailing comma is not allowed in import()");
		...
		pp$52.parseNew = function() {
			...
			if (this.eat(types$12.parenL)) node.arguments = this.parseExprList(types$12.parenR, this.options.ecmaVersion >= 8, false);
...
`),
		...
		pp$52.parseTemplate = function(ref3) {
			...
			node.quasis = [curElt];
			while (!curElt.tail) {
				if (this.type === types$12.eof) this.raise(this.pos, "Unterminated template literal");
		...
		pp$52.parseObj = function(isPattern, refDestructuringErrors) {
			...
			while (!this.eat(types$12.braceR)) {
				if (!first) {
					this.expect(types$12.comma);
					if (this.options.ecmaVersion >= 5 && this.afterTrailingComma(types$12.braceR)) break;
				} else first = false;
		...
		pp$52.parseProperty = function(isPattern, refDestructuringErrors) {
			...
			if (this.options.ecmaVersion >= 9 && this.eat(types$12.ellipsis)) {
				...
				if (this.type === types$12.comma && refDestructuringErrors && refDestructuringErrors.trailingComma < 0) refDestructuringErrors.trailingComma = this.start;
			...
			if (this.options.ecmaVersion >= 6) {
				...
				if (!isPattern) isGenerator = this.eat(types$12.star);
			}
			var containsEsc = this.containsEsc;
		...
		pp$52.parsePropertyValue = function(prop, isPattern, isGenerator, isAsync, startPos, startLoc, refDestructuringErrors, containsEsc) {
			if ((isGenerator || isAsync) && this.type === types$12.colon) this.unexpected();
		...
		pp$52.parseFunctionBody = function(node, isArrowFunction, isMethod, forInit) {
			...
			if (isExpression) {
				...
				if (this.strict && node.id) this.checkLValSimple(node.id, BIND_OUTSIDE2);
		...
		pp$52.isSimpleParamList = function(params) {
			for (var i3 = 0, list3 = params; i3 < list3.length; i3 += 1) if (list3[i3].type !== "Identifier") return false;
			return true;
		};
		...
		pp$52.parseExprList = function(close, allowTrailingComma, allowEmpty, refDestructuringErrors) {
			...
			while (!this.eat(close)) {
				...
				var elt = void 0;
				if (allowEmpty && this.type === types$12.comma) elt = null;
				else if (this.type === types$12.ellipsis) {
					elt = this.parseSpread(refDestructuringErrors);
					if (refDestructuringErrors && this.type === types$12.comma && refDestructuringErrors.trailingComma < 0) refDestructuringErrors.trailingComma = this.start;
		...
		pp$52.parseIdentNode = function() {
			var node = this.startNode();
			if (this.type === types$12.name) node.name = this.value;
		...
		pp$52.parsePrivateIdent = function() {
			var node = this.startNode();
			if (this.type === types$12.privateId) node.name = this.value;
			else this.unexpected();
			this.next();
		...
		pp$42.curPosition = function() {
			if (this.options.locations) return new Position3(this.curLine, this.pos - this.lineStart);
		};
		...
		pp$32.declareName = function(name, bindingType, pos) {
			...
			if (bindingType === BIND_LEXICAL2) {
				...
				if (this.inModule && scope.flags & SCOPE_TOP2) delete this.undefinedExports[name];
			} else if (bindingType === BIND_SIMPLE_CATCH2) this.currentScope().lexical.push(name);
				...
				if (this.inModule && scope$3.flags & SCOPE_TOP2) delete this.undefinedExports[name];
				if (scope$3.flags & SCOPE_VAR2) break;
			}
		...
		pp$32.currentVarScope = function() {
			for (var i3 = this.scopeStack.length - 1;; i3--) {
				var scope = this.scopeStack[i3];
				if (scope.flags & SCOPE_VAR2) return scope;
			}
		};
		pp$32.currentThisScope = function() {
			for (var i3 = this.scopeStack.length - 1;; i3--) {
				var scope = this.scopeStack[i3];
				if (scope.flags & SCOPE_VAR2 && !(scope.flags & SCOPE_ARROW2)) return scope;
			}
		};
		var Node3 = function Node(parser, pos, loc) {
			...
			this.start = pos;
			this.end = 0;
			if (parser.options.locations) this.loc = new SourceLocation3(parser, loc);
		...
		var unicodeScriptValues2 = {
			...
			14: ecma13ScriptValues2 + " Hrkt Katakana_Or_Hiragana Kawi Nag_Mundari Nagm Unknown Zzzz"
		};
		var data2 = {};
		...
		RegExpValidationState3.prototype.at = function at(i3, forceU) {
			...
			if (!(forceU || this.switchU) || c <= 55295 || c >= 57344 || i3 + 1 >= l) return c;
		...
		RegExpValidationState3.prototype.nextIndex = function nextIndex(i3, forceU) {
			...
			if (!(forceU || this.switchU) || c <= 55295 || c >= 57344 || i3 + 1 >= l || (next = s.charCodeAt(i3 + 1)) < 56320 || next > 57343) return i3 + 1;
			return i3 + 2;
		};
		...
		RegExpValidationState3.prototype.eatChars = function eatChars(chs, forceU) {
			...
			for (var i3 = 0, list3 = chs; i3 < list3.length; i3 += 1) {
				...
				if (current2 === -1 || current2 !== ch) return false;
		...
		pp$12.validateRegExpFlags = function(state) {
			...
			for (var i3 = 0; i3 < flags.length; i3++) {
				...
				if (flags.indexOf(flag, i3 + 1) > -1) this.raise(state.start, "Duplicate regular expression flag");
				if (flag === "u") u = true;
				if (flag === "v") v = true;
			}
		...
		pp$12.regexp_disjunction = function(state) {
			...
			if (trackDisjunction) state.branchID = new BranchID3(state.branchID, null);
		...
		pp$12.regexp_eatPatternCharacters = function(state) {
			var start = state.pos;
			var ch = 0;
			while ((ch = state.current()) !== -1 && !isSyntaxCharacter2(ch)) state.advance();
		...
		pp$12.regexp_groupSpecifier = function(state) {
			if (state.eat(63)) {
				...
				if (known) {
					if (trackDisjunction) {
						for (var i3 = 0, list3 = known; i3 < list3.length; i3 += 1) if (!list3[i3].separatedFrom(state.branchID)) state.raise("Duplicate capture group name");
		...
		pp$12.regexp_eatRegExpIdentifierName = function(state) {
			...
			if (this.regexp_eatRegExpIdentifierStart(state)) {
				...
				while (this.regexp_eatRegExpIdentifierPart(state)) state.lastStringValue += codePointToString2(state.lastIntValue);
				return true;
			}
			return false;
		};
		...
		pp$12.regexp_eatRegExpUnicodeEscapeSequence = function(state, forceU) {
			...
			if (state.eat(117)) {
				...
				if (switchU && state.eat(123) && this.regexp_eatHexDigits(state) && state.eat(125) && isValidUnicode2(state.lastIntValue)) return true;
		...
		pp$12.regexp_eatCharacterClassEscape = function(state) {
			...
			if (state.switchU && this.options.ecmaVersion >= 9 && ((negate = ch === 80) || ch === 112)) {
				...
				if (state.eat(123) && (result = this.regexp_eatUnicodePropertyValueExpression(state)) && state.eat(125)) {
					if (negate && result === CharSetString2) state.raise("Invalid property name");
					return result;
				}
		...
		pp$12.regexp_validateUnicodePropertyNameAndValue = function(state, name, value) {
			if (!hasOwn2(state.unicodeProperties.nonBinary, name)) state.raise("Invalid property name");
		...
		pp$12.regexp_validateUnicodePropertyNameOrValue = function(state, nameOrValue) {
			if (state.unicodeProperties.binary.test(nameOrValue)) return CharSetOk2;
			if (state.switchV && state.unicodeProperties.binaryOfStrings.test(nameOrValue)) return CharSetString2;
			state.raise("Invalid property name");
		};
		...
		pp$12.regexp_eatCharacterClass = function(state) {
			if (state.eat(91)) {
				...
				if (negate && result === CharSetString2) state.raise("Negated character class may contain strings");
				return true;
			}
			return false;
		};
		pp$12.regexp_classContents = function(state) {
			if (state.current() === 93) return CharSetOk2;
		...
		pp$12.regexp_eatClassAtom = function(state) {
			...
			if (state.eat(92)) {
				...
				if (state.switchU) {
					...
					if (ch$1 === 99 || isOctalDigit2(ch$1)) state.raise("Invalid class escape");
					state.raise("Invalid escape");
				}
				state.pos = start;
			}
		...
		pp$12.regexp_classSetExpression = function(state) {
			...
			if (this.regexp_eatClassSetRange(state));
				...
				if (subResult === CharSetString2) result = CharSetString2;
				var start = state.pos;
				while (state.eatChars([38, 38])) {
					if (state.current() !== 38 && (subResult = this.regexp_eatClassSetOperand(state))) {
						if (subResult !== CharSetString2) result = CharSetOk2;
						continue;
					}
		...
		pp$12.regexp_eatClassSetOperand = function(state) {
			if (this.regexp_eatClassSetCharacter(state)) return CharSetOk2;
		...
		pp$12.regexp_classStringDisjunctionContents = function(state) {
			...
			while (state.eat(124)) if (this.regexp_classString(state) === CharSetString2) result = CharSetString2;
			return result;
		};
		...
		pp$12.regexp_eatClassSetCharacter = function(state) {
			...
			var ch = state.current();
			if (ch < 0 || ch === state.lookahead() && isClassSetReservedDoublePunctuatorCharacter2(ch)) return false;
			if (isClassSetSyntaxCharacter2(ch)) return false;
			state.advance();
			state.lastIntValue = ch;
			return true;
		};
		...
		var Token3 = function Token(p) {
			...
			this.start = p.start;
			this.end = p.end;
			if (p.options.locations) this.loc = new SourceLocation3(p, p.startLoc, p.endLoc);
		...
		pp2.next = function(ignoreEscapeSequenceInKeyword) {
			...
			if (this.options.onToken) this.options.onToken(new Token3(this));
		...
		if (typeof Symbol !== "undefined") pp2[Symbol.iterator] = function() {
			var this$1$1 = this;
		...
		pp2.nextToken = function() {
			...
			if (this.pos >= this.input.length) return this.finishToken(types$12.eof);
			if (curContext.override) return curContext.override(this);
			else this.readToken(this.fullCharCodeAtPos());
		};
		pp2.readToken = function(code) {
			if (isIdentifierStart2(code, this.options.ecmaVersion >= 6) || code === 92) return this.readWord();
		...
		pp2.skipBlockComment = function() {
			...
			this.pos = end + 2;
			if (this.options.locations) for (var nextBreak = void 0, pos = start; (nextBreak = nextLineBreak2(this.input, pos, this.pos)) > -1;) {
				++this.curLine;
		...
		pp2.skipLineComment = function(startSkip) {
			...
			while (this.pos < this.input.length && !isNewLine2(ch)) ch = this.input.charCodeAt(++this.pos);
			...
			loop: while (this.pos < this.input.length) {
				...
				switch (ch) {
					...
					case 47:
						...
						break;
					default: if (ch > 8 && ch < 14 || ch >= 5760 && nonASCIIwhitespace2.test(String.fromCharCode(ch))) ++this.pos;
					else break loop;
				}
			}
		};
		...
		pp2.readToken_slash = function() {
			...
			if (next === 61) return this.finishOp(types$12.assign, 2);
		...
		pp2.readToken_mult_modulo_exp = function(code) {
			...
			if (next === 61) return this.finishOp(types$12.assign, size + 1);
		...
		pp2.readToken_pipe_amp = function(code) {
			...
			if (next === code) {
				if (this.options.ecmaVersion >= 12) {
					if (this.input.charCodeAt(this.pos + 2) === 61) return this.finishOp(types$12.assign, 3);
				}
		...
		pp2.readToken_caret = function() {
			if (this.input.charCodeAt(this.pos + 1) === 61) return this.finishOp(types$12.assign, 2);
		...
		pp2.readToken_lt_gt = function(code) {
			...
			if (next === code) {
				...
				if (this.input.charCodeAt(this.pos + size) === 61) return this.finishOp(types$12.assign, size + 1);
		...
		pp2.readToken_eq_excl = function(code) {
			var next = this.input.charCodeAt(this.pos + 1);
			if (next === 61) return this.finishOp(types$12.equality, this.input.charCodeAt(this.pos + 2) === 61 ? 3 : 2);
		...
		pp2.readToken_question = function() {
			...
			if (ecmaVersion3 >= 11) {
				...
				if (next === 46) {
					var next2 = this.input.charCodeAt(this.pos + 2);
					if (next2 < 48 || next2 > 57) return this.finishOp(types$12.questionDot, 2);
				}
				if (next === 63) {
					if (ecmaVersion3 >= 12) {
						if (this.input.charCodeAt(this.pos + 2) === 61) return this.finishOp(types$12.assign, 3);
		...
		pp2.readToken_numberSign = function() {
			...
			if (ecmaVersion3 >= 13) {
				...
				if (isIdentifierStart2(code, true) || code === 92) return this.finishToken(types$12.privateId, this.readWord1());
			}
		...
		pp2.getTokenFromCode = function(code) {
			switch (code) {
				...
				case 126: return this.finishOp(types$12.prefix, 1);
		...
		pp2.readRegexp = function() {
			...
			for (;;) {
				...
				if (lineBreak2.test(ch)) this.raise(start, "Unterminated regular expression");
				if (!escaped) {
					if (ch === "[") inClass = true;
			...
			return this.finishToken(types$12.regexp, {
				pattern,
				flags,
				value
			});
		...
		pp2.readInt = function(radix, len, maybeLegacyOctalNumericLiteral) {
			...
			for (var i3 = 0, e = len == null ? Infinity : len; i3 < e; ++i3, ++this.pos) {
				...
				if (allowSeparators && code === 95) {
					...
					if (i3 === 0) this.raiseRecoverable(this.pos, "Numeric separator is not allowed at the first of digits");
					lastCode = code;
					continue;
				}
		...
		pp2.readRadixNumber = function(radix) {
			...
			if (this.options.ecmaVersion >= 11 && this.input.charCodeAt(this.pos) === 110) {
				...
				++this.pos;
			} else if (isIdentifierStart2(this.fullCharCodeAtPos())) this.raise(this.pos, "Identifier directly after number");
		...
		pp2.readNumber = function(startsWithDot) {
			...
			if (!octal && !startsWithDot && this.options.ecmaVersion >= 11 && next === 110) {
				...
				++this.pos;
				if (isIdentifierStart2(this.fullCharCodeAtPos())) this.raise(this.pos, "Identifier directly after number");
		...
		pp2.readString = function(quote) {
			...
			for (;;) {
				...
				if (ch === 92) {
					...
					if (isNewLine2(ch)) this.raise(this.start, "Unterminated string constant");
					++this.pos;
				}
			}
			out += this.input.slice(chunkStart, this.pos++);
		...
		pp2.tryReadTemplateToken = function() {
			...
			try {
				...
				if (err === INVALID_TEMPLATE_ESCAPE_ERROR2) this.readInvalidTemplateToken();
				else throw err;
			}
			this.inTemplateElement = false;
		};
		pp2.invalidStringToken = function(position, message) {
			if (this.inTemplateElement && this.options.ecmaVersion >= 9) throw INVALID_TEMPLATE_ESCAPE_ERROR2;
...
`;
		...
		pp2.readInvalidTemplateToken = function() {
			for (; this.pos < this.input.length; this.pos++) switch (this.input[this.pos]) {
				...
				case "`": return this.finishToken(types$12.invalidTemplate, this.input.slice(this.start, this.pos));
...
`;
				...
				case 117: return codePointToString2(this.readCodePoint());
				case 116: return "	";
				case 98: return "\b";
				case 118: return "\v";
				case 102: return "\f";
		...
		pp2.readWord1 = function() {
			...
			while (this.pos < this.input.length) {
				...
				if (isIdentifierChar2(ch, astral)) this.pos += ch <= 65535 ? 1 : 2;
				else if (ch === 92) {
					...
					if (!(first ? isIdentifierStart2 : isIdentifierChar2)(esc, astral)) this.invalidStringToken(escStart, "Invalid Unicode escape");
		...
		pp2.readWord = function() {
			...
			if (this.keywords.test(word)) type = keywords2[word];
...
`;
			...
			jsx_parseOpeningElementAt(startPos, startLoc) {
				...
				if (options.autoCloseVoidElements && nodeName && [
					"area",
					"base",
					"br",
					"col",
					...
					"track",
					"wbr"
				].includes(nodeName.name)) node.selfClosing = true;
...
function getLineInfo(input, offset2) {
	for (var line = 1, cur = 0;;) {
		...
		if (nextBreak < 0) return new Position(line, offset2 - cur);
		++line;
		cur = nextBreak;
	}
}
...
RegExpValidationState.prototype.nextIndex = function nextIndex(i2, forceU) {
	...
	if (!(forceU || this.switchU) || c <= 55295 || c >= 57344 || i2 + 1 >= l || (next = s.charCodeAt(i2 + 1)) < 56320 || next > 57343) return i2 + 1;
	return i2 + 2;
};
...
pp$1.validateRegExpFlags = function(state) {
	...
	for (var i2 = 0; i2 < flags.length; i2++) {
		...
		if (validFlags.indexOf(flag) === -1) this.raise(state.start, "Invalid regular expression flag");
		if (flags.indexOf(flag, i2 + 1) > -1) this.raise(state.start, "Duplicate regular expression flag");
		if (flag === "u") u = true;
		if (flag === "v") v = true;
	}
	if (this.options.ecmaVersion >= 15 && u && v) this.raise(state.start, "Invalid regular expression flag");
...
pp$1.regexp_groupSpecifier = function(state) {
	if (state.eat(63)) {
		...
		if (known) {
			if (trackDisjunction) {
				for (var i2 = 0, list2 = known; i2 < list2.length; i2 += 1) if (!list2[i2].separatedFrom(state.branchID)) state.raise("Duplicate capture group name");
			} else state.raise("Duplicate capture group name");
		}
		if (trackDisjunction) (known || (state.groupNames[state.lastStringValue] = [])).push(state.branchID);
		else state.groupNames[state.lastStringValue] = true;
...
pp.readInt = function(radix, len, maybeLegacyOctalNumericLiteral) {
	...
	for (var i2 = 0, e = len == null ? Infinity : len; i2 < e; ++i2, ++this.pos) {
		...
		if (allowSeparators && code === 95) {
			if (isLegacyOctalNumericLiteral) this.raiseRecoverable(this.pos, "Numeric separator is not allowed in legacy octal numeric literals");
			if (lastCode === 95) this.raiseRecoverable(this.pos, "Numeric separator must be exactly one underscore");
			if (i2 === 0) this.raiseRecoverable(this.pos, "Numeric separator is not allowed at the first of digits");
			lastCode = code;
			continue;
		}
		if (code >= 97) val = code - 97 + 10;
...
var parseStyle = (style) => {
	switch (typeof style) {
		case "string": return style.split(";").filter((r) => r).reduce((map, rule) => {
...
var JsxParser = class extends import_react.Component {
	...
	ParsedChildren = null;
	#parseJSX = (jsx) => {
		const parser = Parser.extend(AcornJSX.default({ autoCloseVoidElements: this.props.autoCloseVoidElements }));
	...
	#parseExpression = (expression, scope) => {
		switch (expression.type) {
			...
			case "JSXElement":
			case "JSXFragment": return this.#parseElement(expression, scope);
			case "JSXExpressionContainer": return this.#parseExpression(expression.expression, scope);
			case "JSXText":
				const key = this.props.disableKeyGeneration ? void 0 : randomHash();
				return this.props.disableFragments ? expression.value : /* @__PURE__ */ import_react.createElement(import_react.Fragment, { key }, expression.value);
			case "ArrayExpression": return expression.elements.map((ele) => this.#parseExpression(ele, scope));
			case "BinaryExpression":
				...
				switch (expression.operator) {
					...
					case ">=":
						...
						break;
					default:
						this.props.onError(/* @__PURE__ */ new Error(`Unsupported binary operator: ${expression.operator}`));
						return;
				}
			...
			case "CallExpression":
				...
				if (parsedCallee === void 0) {
					...
					this.props.onError(/* @__PURE__ */ new Error(`The expression '${expression.callee}' could not be resolved, resulting in an undefined return value.`));
					return;
				}
				return parsedCallee(...expression.arguments.map((arg) => this.#parseExpression(arg, scope)));
			case "ConditionalExpression": return this.#parseExpression(expression.test, scope) ? this.#parseExpression(expression.consequent, scope) : this.#parseExpression(expression.alternate, scope);
			case "ExpressionStatement": return this.#parseExpression(expression.expression, scope);
			case "Identifier":
				if (expression.name === "Infinity") return Infinity;
				if (expression.name === "-Infinity") return -Infinity;
				if (expression.name === "NaN") return NaN;
				if (scope && expression.name in scope) return handleNaN(scope[expression.name]);
			...
			case "LogicalExpression":
				...
				if (expression.operator === "&&" && left || expression.operator === "||" && !left) return this.#parseExpression(expression.right, scope);
				return false;
			case "MemberExpression": return this.#parseMemberExpression(expression, scope);
			case "ChainExpression": return this.#parseChainExpression(expression, scope);
			case "ObjectExpression":
				const object = {};
				...
				return object;
			case "TemplateElement": return expression.value.cooked;
			case "TemplateLiteral": return [...expression.expressions, ...expression.quasis].sort((a, b) => {
			...
			case "ArrowFunctionExpression":
				if (expression.async || expression.generator) this.props.onError?.(/* @__PURE__ */ new Error("Async and generator arrow functions are not supported."));
				return (...args) => {
			...
			default:
				this.props.onError(/* @__PURE__ */ new Error(`The expression type '${expression.type}' is not supported.`));
				return;
		}
	};
	#parseChainExpression = (expression, scope) => {
		try {
			...
			if (error instanceof NullishShortCircuit) return;
			throw error;
		}
	};
	#parseMemberExpression = (expression, scope) => {
		...
		let property;
		if (expression.computed) property = this.#parseExpression(expression.property, scope);
		else if (expression.property.type === "Identifier") property = expression.property.name;
		else {
			this.props.onError(/* @__PURE__ */ new Error("Only simple MemberExpressions are supported."));
			return;
		}
		...
		try {
			...
			this.props.onError(/* @__PURE__ */ new Error(`The property '${property}' could not be resolved on the object '${object}'.`));
			return;
		}
	...
	#parseElement = (element, scope) => {
		...
		if (/^(html|head|body)$/i.test(name)) return childNodes.map((c) => this.#parseElement(c, scope));
		const tagName = name.trim().toLowerCase();
		if (blacklistedTags.indexOf(tagName) !== -1) {
			onError(/* @__PURE__ */ new Error(`The tag <${name}> is blacklisted, and will not be rendered.`));
			return null;
		}
		if (name !== "" && !resolvePath(components, name)) {
			if (componentsOnly) {
				onError(/* @__PURE__ */ new Error(`The component <${name}> is unrecognized, and will not be rendered.`));
			...
			if (!allowUnknownElements && document.createElement(name) instanceof HTMLUnknownElement) {
				onError(/* @__PURE__ */ new Error(`The tag <${name}> is unrecognized in this browser, and will not be rendered.`));
				return this.props.renderUnrecognized(name);
			}
		}
		let children;
		let component = element.type === "JSXElement" ? resolvePath(components, name) : import_react.Fragment;
		if (component || canHaveChildren(name)) {
			children = childNodes.map((node) => this.#parseExpression(node, scope));
			if (name.includes(".")) {
				const componentPath = name.split(".").reduce((acc, part) => acc ? acc[part] : components?.[part], null);
				if (componentPath) component = componentPath;
			}
			if (!component && !canHaveWhitespace(name)) children = children.filter((child) => typeof child !== "string" || !/^\s*$/.test(child));
			const childFn = children.find((child) => typeof child === "function");
			if (children.length === 0) children = void 0;
			else if (childFn) children = childFn;
			else if (children.length === 1) [children] = children;
			else if (children.length > 1 && !this.props.disableKeyGeneration) children = children.map((child, key) => child?.type && !child?.key ? {
				...child,
				key: child.key || key
			} : child);
		}
		const props = { key: this.props.disableKeyGeneration ? void 0 : randomHash() };
		attributes.forEach((expr) => {
			if (expr.type === "JSXAttribute") {
				...
				if (blacklistedAttrs.filter((re) => re.test(attributeName)).length === 0) props[attributeName] = value;
				...
				if (typeof value === "object") Object.keys(value).forEach((rawName) => {
					const attributeName = attributeNames_default[rawName] || rawName;
					if (blacklistedAttrs.filter((re) => re.test(attributeName)).length === 0) props[attributeName] = value[rawName];
				});
			}
		});
		if (typeof props.style === "string") props.style = parseStyle(props.style);
		const lowerName = name.toLowerCase();
		if (lowerName === "option") children = children.props.children;
		return import_react.createElement(component || lowerName, props, children);
	};
	render() {
		...
		return this.props.renderInWrapper ? /* @__PURE__ */ import_react.createElement("div", { className }, this.ParsedChildren) : this.ParsedChildren;
	}
};
